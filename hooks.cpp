...
stFile* NvFOpen_hook(const char* r0, const char* r1, int r2, int r3)
{
	...
	// ----------------------------
	/*if(!strcmp(r1, cryptor::create("data/paths/tracks2.dat", 23).decrypt()))
	{
		sprintf(path, SAMPFiles[9].decrypt(), g_pszStorage);
		Log("Loading %s..", SAMPFiles[9].decrypt());
		goto open;
	}
	// ----------------------------
	if(!strcmp(r1, cryptor::create("data/paths/tracks4.dat", 23).decrypt()))
	{
		sprintf(path, SAMPFiles[10].decrypt(), g_pszStorage);
		Log("Loading %s..", SAMPFiles[10].decrypt());
		goto open;
	}*/
	// ----------------------------
...
void MobileMenu__Render_hook(uintptr_t thiz)
{
	...
	if(pGUI)CFont::PrintString(pGUI->ScaleX(5), pGUI->ScaleY(5), cryptor::create("Made on Earth by Humans", 24).decrypt());
}
...
{	
	...
	CPatch::InlineHook(g_libGTASA, 0x241D94, &NvUtilInit_hook, &NvUtilInit); // было два раза ;;
