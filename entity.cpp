...
{
	if (m_pEntity)
		return (uintptr_t)m_pEntity->m_pRwObject;
...
float CEntity::GetDistanceFromPoint(VECTOR vec)
{
	MATRIX4X4 matThis;
	float fSX, fSY, fSZ;

	GetMatrix(&matThis);
	fSX = (matThis.pos.X - vec.X) * (matThis.pos.X - vec.X);
	fSY = (matThis.pos.Y - vec.Y) * (matThis.pos.Y - vec.Y);
	fSZ = (matThis.pos.Z - vec.Z) * (matThis.pos.Z - vec.Z);
...
{
  	if(m_pEntity && !IsGameEntityArePlaceable(m_pEntity) && ModelInfoGetColSphereRadius(m_pEntity->nModelIndex) > 0.0)
  	{
		if(bTrue)
